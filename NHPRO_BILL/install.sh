#!/usr/bin/env bash
# =============================================================================
#  NHPRO_BILL — Installer VPS (clone dari GitHub publik)
#  Stack : Node.js + Express, MariaDB, pm2   |   Target: Ubuntu 22/24 & Debian 11/12
#
#  Pakai (one-liner):
#     wget -qO- https://raw.githubusercontent.com/idpanyoet/NHPRO_BILL-Project/master/install.sh | sudo bash
#
...
GITHUB_REPO="${GITHUB_REPO:-NHPRO_BILL-Project}"
GITHUB_BRANCH="${GITHUB_BRANCH:-master}"

INSTALL_DIR="${INSTALL_DIR:-/opt/nhpro_bill}"
APP_PORT="${APP_PORT:-3000}"
PM2_NAME="${PM2_NAME:-nhpro_bill}"
NODE_MAJOR="${NODE_MAJOR:-20}"

DB_NAME="${DB_NAME:-nhpro_bill_radius}"
DB_USER="${DB_USER:-nhpro_bill}"
...
DOMAIN="${DOMAIN:-}"                    # mis. nhprobill.domain.com → pasang Nginx+SSL (opsional)
...
# Dipakai fitur 'VPN Server' NHPRO_BILL (app memanggil wg/wg-quick; cek xl2tpd & ipsec).
...
if [[ "${INSTALL_VPN,,}" == y* ]]; then
  ...
  ok "Tools VPN terpasang (${VPN_OK} paket) — konfigurasi via menu VPN NHPRO_BILL"
else
...
VALUES ('${ADMIN_USER}', 'Super Admin', 'admin@nhprobill.id', '${ADMIN_HASH}', 'superadmin', 1)
...
if [[ "${INSTALL_RADIUS,,}" == y* ]]; then
  ...
  if [ -z "$RADDIR" ] || [ ! -f "$SQLMOD" ]; then
    ...
    # 1) Modul sql → arahkan ke DB NHPRO_BILL (dialect mysql)
    sed -i \
...
PY
    ...
    if freeradius -XC >/tmp/nhpro-bill-fr-check.log 2>&1; then
      ...
      tail -15 /tmp/nhpro-bill-fr-check.log | sed 's/^/    /'
      info "Debug manual: ${C_B}freeradius -X${C_R}"
    fi
    info "Daftarkan MikroTik di menu RADIUS/NAS NHPRO_BILL, lalu: systemctl restart freeradius"
  fi
fi
...
if [[ "${INSTALL_RADIUS,,}" == y* || "${INSTALL_VPN,,}" == y* ]]; then
  ...
  echo 'net.ipv4.ip_forward=1' > /etc/sysctl.d/99-nhpro-bill.conf
...
if [ -n "$DOMAIN" ]; then
  ...
  cat > /etc/nginx/sites-available/nhprobill <<NGINX
server {
    listen 80;
    server_name ${DOMAIN};
...
NGINX
  ln -sf /etc/nginx/sites-available/nhprobill /etc/nginx/sites-enabled/nhprobill
