...
```bash
wget -qO- https://raw.githubusercontent.com/idpanyoet/NHPRO_BILL-Project/master/update.sh | sudo bash
```

Installer otomatis memasang **Node.js 20, pm2, MariaDB**, membuat database, meng-clone aplikasi ke `/opt/nhpro_bill`, membuat file `.env` (JWT digenerate otomatis), mengimpor schema, lalu menjalankan aplikasi via **pm2** dengan nama `nhpro_bill`. Setelah selesai, akses dashboard di `http://IP-VPS:3000`.
...
> ```bash
> curl -fsSL https://raw.githubusercontent.com/idpanyoet/NHPRO_BILL-Project/master/install.sh | sudo ADMIN_PASS=PasswordKuatku bash
> ```
...
Konfigurasi utama ada di `/opt/nhpro_bill/backend/.env` (dibuat otomatis oleh installer). Yang penting:
...
<p align="center"><sub>NHPRO_BILL © 2026 — dibuat untuk komunitas ISP & RT/RW Net Indonesia.</sub></p>
